...
export function GetAdsModal(props: GetAdsModalProps) {
  ...
  useEffect(() => {
    axios(`${url}game/${props.id}/ads`).then((response) => {
    ...
    <Dialog.Portal>
      ...
      <Dialog.Content className="fixed bg-[#2a2634] py-8 px-10 text-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg max-w-[600px] w-full shadow-lg shadow-black/25">
        <Dialog.Title className="text-3xl text-white font-black">
          <p>
            <a className="text-transparent bg-nlw-gradient bg-clip-text">
              Duos
            </a>{' '}
            para
            <br />
        ...
        <div className="mt-8">
          {ads.map((ad) => {
            return (
              <div
                className="mt-2 w-fit p-2 border-2 border-solid border-zinc-400 rounded-md float-left mx-1"
                key={ad.id}
              >
                <p>{`Duo: ${ad.name} xp: ${ad.yearsPlaying}y`}</p>
              </div>
            );
          })}
        </div>
